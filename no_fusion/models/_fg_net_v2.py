...
from .focus_gated import FocusAttnGatingBlock_v2
...
def decoder_fg_resnet50_build_decoder_v2(encoder: Model, hiper):
    enc = load_pretrained_weights(encoder, hiper)
    z = enc.output

    x = upsampling('up_conv', z, hiper, filt * 16, 'bottleneck')
    fg1 = FocusAttnGatingBlock_v2(encoder.get_layer(name=skip_layer['resnet50'][0]).output,
                                  x, filt * 16, fus='par', name='fg1')  # 1024
    ...
    fg2 = FocusAttnGatingBlock_v2(encoder.get_layer(name=skip_layer['resnet50'][1]).output,
                                  x, filt * 8, fus='par', name='fg2')  # 512
    ...
    x = upsampling('up_conv', x_out1, hiper, filt * 4, 'decod3')
    fg3 = FocusAttnGatingBlock_v2(encoder.get_layer(name=skip_layer['resnet50'][2]).output,
                               x, filt * 4, fus='par', name='fg3')
    ...
    x = upsampling('up_conv', x_out2, hiper, filt, 'decod4')
    fg4 = FocusAttnGatingBlock_v2(encoder.get_layer(name=skip_layer['resnet50'][3]).output,
                               x, filt, fus='par', name='fg4')
    ...
    x = upsampling('up_conv', x_out3, hiper, filt // 2, 'decod5')
